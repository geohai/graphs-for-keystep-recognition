...
def load_labels(actions, root_data, annotation_dataset, video_id, sample_rate=1, feature=None, load_raw=False, load_descriptions=False, verbose=0):
    if annotation_dataset == '50salads' or annotation_dataset == 'egoexo':
        ...
        if len(new_labels) < feature.shape[0]:
            ...
            new_labels.append(mode(last_window))
        label = new_labels

    else:
        label = load_egoexo_omnivideo_integer_labels(video_id, actions, root_data=root_data, dataset=annotation_dataset, feature=feature, load_raw=load_raw, verbose=verbose)

    return label


def load_egoexo_omnivideo_integer_labels(video_id, actions, root_data='../data', dataset='egoexo',feature=None, load_raw=False, verbose=0):
    """
    ...
    loaded = os.path.join(root_data, f'annotations/{dataset}/groundTruth/{video_id}.txt')
    if verbose == 1:
        print(f'Length of loaded labels: {len(label)}')
    # except:
    ...
    # NOTE: this only works when the omnivore features are the features. Segmentwise features doesn't work.
    if load_raw == False:
        new_labels = [scipy.stats.mode(label[i:i+32])[0] for i in range(0, len(label), 16) if i+32 < len(label)]
        # add last label to the end
        if len(new_labels) < feature.shape[0]:
