...
def generate_temporal_h_graph(data_file, args, path_graphs, actions, train_ids, all_ids, list_multiview_data_files=[], cliptext_data_file=''):
    """
    ...
    for i in range(num_frame):
        for j in range(num_frame):
            ...
            if abs(frame_diff) <= args.tauf:
                ...
                node_source_t.append(i)
                node_target_t.append(j)
                edge_attr_t.append(np.sign(frame_diff))
                ...
                if frame_diff == 0:
                    ...
                    if video_id in train_ids:
                        node_source_to.append(i)
                        node_target_to.append(j)
            ...
            # This can help reduce over-segmentation of predictions in some cases
                ...
                if (frame_diff % skip == 0) and (abs(frame_diff) <= skip*args.tauf):
                    ...
                    for k in range(num_view):
                        nodetu_source.append(i+num_frame)
    ...
    graphs = HeteroData()
    graphs.g = all_ids.index(video_id),
    graphs['o'].x = torch.tensor(np.array(feature, dtype=np.float32), dtype=torch.float32)
    graphs['o'].y = torch.tensor(np.array(label, dtype=np.int16)[::args.sample_rate], dtype=torch.long)
    graphs['o', 'oo', 'o'].edge_index = torch.tensor(np.array([node_source, node_target], dtype=np.int32), dtype=torch.long)
    graphs['o', 'oo', 'o'].edge_attr = torch.tensor(edge_attr, dtype=torch.float32)
    #if cliptext_data_file:
    if True:
        feature_cliptext = np.load(cliptext_data_file)
        graphs['t'].x = torch.tensor(np.array(feature_cliptext, dtype=np.float32), dtype=torch.float32)
        #graphs['t'].y = torch.tensor(np.array(label, dtype=np.int16)[::args.sample_rate], dtype=torch.long)
        graphs['t', 'tt', 't'].edge_index = torch.tensor(np.array([node_source_t, node_target_t], dtype=np.int32), dtype=torch.long)
        graphs['t', 'tt', 't'].edge_attr = torch.tensor(edge_attr_t, dtype=torch.float32)
        graphs['t', 'to', 'o'].edge_index = torch.tensor(np.array([node_source_to, node_target_to], dtype=np.int32), dtype=torch.long)
        graphs['t', 'to', 'o'].edge_attr = torch.zeros((len(node_source_to),))
